...
template<size_t DOF>
  ...
  public:
    ...
    //Rieman
    void 
    JointPositions(std::stringstream & data);
...
template<size_t DOF>
  void WamNode<DOF>::init(ProductManager& pm)
  {
    ...
    JointValsFilename = "/home/robot/rieman_ws/data/JointVals_rieman.csv";
...
template<size_t DOF>
  void WamNode<DOF>::JointPosForRecording()
  {
    ...
    systems::Ramp time(pmPtr->getExecutionManager());
  	...
  	systems::TupleGrouper<double, jp_type> tg;//, jv_type, jt_type, cp_type, Eigen::Quaterniond> tg;
    ...
    typedef boost::tuple<double, jp_type> tuple_type;//, jv_type, jt_type, cp_type, Eigen::Quaterniond> tuple_type;

    char tmpFile[] = "/tmp/JtPosXXXXXX";
    if (mkstemp(tmpFile) == -1) {
        printf("ERROR: Couldn't create temporary file!\n");
    }
	else
		printf("tmpFile's name: %s.\n", tmpFile);
...
template<size_t DOF>
  ...
  {
    std::cout << ("JointTrajectoryVelocityMove CALLBACK ****************") << std::endl;
    typedef boost::tuple<double, jp_type> jp_sample_type;
    ...
    //Build time stamps for traveling time and desired velocity
    ...
    // first move to the start position
    std::cout << ("MOVING TO THE START OF TRAJ") << std::endl;

    wam_joint_state.name.resize(DOF + 1, "START");
    wam.moveTo(spline.eval(spline.initialS()));
    ...
    wam.moveTo(wam.getJointPositions());
    wam_joint_state.name[DOF] = "FINNISHED";
    //wam.gravityCompensate(req.gravity);
    //ROS_INFO("Gravity Compensation Request: %s", (req.gravity) ? "true" : "false");
    res.status = 1;
    return true;
...
template<size_t DOF>
  void WamNode<DOF>::publishWam()
  {
    while (ros::ok())
    {
        ...
        transform_.setOrigin(tf::Vector3(cp_pub[0], cp_pub[1], cp_pub[2]));
        transform_.setRotation(tf::Quaternion(to_pub.x(), to_pub.y(), to_pub.z(), to_pub.w()));
        br_.sendTransform(tf::StampedTransform(transform_, ros::Time::now(), "robot_base", "robot_end_effector"));
...
template<size_t DOF>
  int wam_main(int argc, char** argv, ProductManager& pm, systems::Wam<DOF>& wam)
  {
    ...
    printf("C: Start gravity Compensator\n");
    printf("H: Go to home position\n");
    printf("G: Get the WAM movement joint position data.\n");
    printf("R: Repeat the movement from the data\n");
    

    while (check && std::cin >> mode)
    {
        switch(mode)
        {
            case 'C': {
            ...
            case 'R': {
                std::cout << std::endl << "Please execute the repeat_trajectory client node and the IMUJointPos_subscriber node." << std::endl;
                check = false;
                break;
            }
            default:
            {
                std::cout << "Wrong choice: " << mode << std::endl;
                std::cin.ignore(1,'\n');
                break;
            }
        }
    ...
    std::cout << std::endl << "Publish wam." << std::endl;
