...
int main(int argc, char **argv)
{
  ...
  start_pose.position.x += 0.0025;
  start_pose.position.z -= 0.0120;
  ...
  int numOfFailures = 0;
  double incrementalTravelLength = 0.004; // 0.005 m
  ...
  while(currLearning) //currEpisode
  {
    ...
    { 
      curr_pose.position.x -=  0 * incrementalTravelLength;
      curr_pose.position.z +=  1 * incrementalTravelLength;
      planAndMove = true;
    }
