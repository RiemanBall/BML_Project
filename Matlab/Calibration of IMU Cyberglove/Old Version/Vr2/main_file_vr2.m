...
resLength = 4*frameNum;

%% Observability
O = zeros(resLength * k, TVStateNum * k + constStateNum);
checkObsv = true;
...
V = zeros(resLength, resLength);
var_measurement_Angle = ones(sensorNum,3) * var_measurement;
var_measurement_quat = r2q(var_measurement_Angle);
for i = 1:sensorNum
    V(4*(i-1)+1:4*i, 4*(i-1)+1:4*i) = diag(var_measurement_quat(i).double);
end
...
%         r_k = residual_vr2(IMUEst, output);                             % Compute the residual
...
%     end
    ...
    costFunh = @(x_est)costFun_vr2( x_est, x_pred, output, frameNum, constStateNum, M, V);
    nonlcon = @(x) mycon(x,frameNum);
    hessianFun = @(x) myHessian( x, output, frameNum, constStateNum, M, V);
    options = optimoptions('fmincon','Algorithm','interior-point','SpecifyObjectiveGradient',true,'HessianFcn',hessianFun);
    A = [];
    b = [];
    Aeq = [];
    beq = [];
    ...
    [x_est,~,~,~,~,hessian] = fmincon(costFunh, x_est, A, b, Aeq, beq, lb, ub, nonlcon, options);
...
constStates_est = x_est1(1:constStateNum,stillK);
constStatesEstMatrix = (reshape(constStates_est,sensorNum,3))';
q_IMUEst = angvec2q(constStatesEstMatrix, 1e-4, sensorNum);
initialFrame = Quaternion;
for i = 1:sensorNum
    figure(figNum)
    hold on
    q_IMUEst(i).plot('color','r');
    q_Placement(i).plot('color','b');
    initialFrame.plot('color','k');
    figNum = figNum + 1;
end
