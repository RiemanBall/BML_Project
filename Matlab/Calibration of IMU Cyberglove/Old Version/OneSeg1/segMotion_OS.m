...
function [placement_angvec, placement_quat, rotAngle, devAngle, IMU_truth, seg_truth] = segMotion_OS(k, frameNum, TVVarNum, stdDev_Config0, stdDev_Orient0, stdDev_Placement0)
...
degreeRange = 90;                   % The range of the joint within which the segment is able to move.
rotAngle = zeros(TVVarNum, k);
rotAngle(end, 2:end) = randMotion_OS(k-1, degreeRange);                 % Create random finger motion in degree with the given range of mobilities
devAngle = segConfig0';
rotAngle = d2r(rotAngle) + devAngle;                                    % Tranform degree to radius and shift the angle with the deviation. Dimension: 3xk

%% Update
for i = 1:k
    [seg_truth(i,:), IMU_truth(i,:)] = configUpdate_OS(placement_quat, rotAngle(:,i), frameNum);
end

%% Deviation from the global frame
for i = 1:k
    for j = 1:frameNum
        seg_truth(i,j) = segOrient0_quat * seg_truth(i,j);
        IMU_truth(i,j) = segOrient0_quat * IMU_truth(i,j);
    end
end