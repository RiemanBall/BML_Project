...
[placement_angvec, placement, fingerMove, IMU_truth, finger_truth] = handMotion(k, frameNum, TVStateNum, stdDev_Config0, stdDev_Orient0, stdDev_Placement0);
RP = placement_angvec';
...
iterateNum = 10;
finger_est = repmat(struct('orient', repmat(Quaternion(),1,3)),k,1);            % Initial estimated configuration of the finger is straight.
IMU_est = repmat(struct('orient', repmat(Quaternion(),1,3)),k,1);               % Initial estimated configuration of the IMU is perfectly aligned with the finger.
...
for i = 2 : stillK
    ...
    finger_est(i).orient = fingerEst;
    IMU_est(i).orient = IMUEst;
    x_est1(:,i) = x_est;
...
r0 = residual(IMU_est(1).orient, IMUOutput(1).orient);                       % Compute the initial residual
rEnd = r_k;
...
for i = 2 : k
    ...
    for j = 1:iterateNum    % Iteration
        ...
        r_k = residual(IMU_est(i).orient, IMUOutput(i).orient);      % Compute the residual
        J = jacob(r_k, x_est2(:,i), finger_est(i).orient, IMU_est(i).orient, IMUOutput(i).orient );    % Compute the Jacobian of the residual
...
%         end
        ...
        h = - H_inv * gradient;
        alpha = lineSearch(x_est2(:,i), x_pred, IMUOutput(i).orient, J, r_k, h, M, V, frameNum, 'soft');
