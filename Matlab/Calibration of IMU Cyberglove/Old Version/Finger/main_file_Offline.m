...
% In the test3, we fixed the global sensor on the hand and assume
% the sensors didn't slide during the record

%% Preprocessing
data = xlsread('dataVideo3.xlsx', 'Sheet1', '', 'basic');
[k, sensorNum] = size(data);           % k is the time index
sensorNum = sensorNum/4;                     % Quaternion has 4 elements
fingerNum = 1;
globalSensorNum = 1;
localSensorNum = sensorNum - globalSensorNum;

IMUOutput = repmat(repmat(Quaternion, 1, localSensorNum), k, 1);
for i = 1:k
    globalSensorOrient = Quaternion(data(i,1:4));
    for j = 1:localSensorNum
        localSensorOrient = Quaternion(data(i, 3*j+1: 3*(j+1) ));
        IMUOutput(i, j) = globalSensorOrient.inv * localSensorOrient;
    end
end

%% Initialization of the constant parameters
stillK = 100;
pLength_const = [5, 3, 1];                                   % Set the length of the user's phalanges
frameNum = localSensorNum / fingerNum;
constStateNum = localSensorNum*3;
...
%% Simulation
% % Initialization of the parameters in the simulation
...
% 
% %% Create hand motion and ground-truth for the simulation
...
% 
% [placement_angvec, placement, rotAngle, IMU_truth, finger_truth] = handMotion(k, frameNum, stdDev_Config0, stdDev_Orient0, stdDev_Placement0);
% RP = placement_angvec';
% 
% %% Simulated hand motion animation
...
M = diag( [var_Placement0 * rRatio * ones(1,constStateNum), (var_Config0 + var_Orient0) * ones(1, TVStateNum)] );
...
for i = 2 : k
    ...
    for j = 1:iterateNum    % Iteration
        ...
        [finger_est(i).orient, IMU_est(i).orient] = configUpdate(constStates_est, TVStates_est, frameNum);        % Estimated motion
