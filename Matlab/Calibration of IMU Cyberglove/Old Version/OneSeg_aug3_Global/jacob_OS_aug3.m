...
function J = jacob_OS_aug3(r, x_est, measurement, constVarNum)
...
for i = 1:4
    %% IMU Placement1
    temp = IMUPlacementEst(2).inv / rotation * q_diag1(i) / measurement(1) * measurement(2);
    dr_du_s1(:, i) = 2 * temp.v';
    
    %% Segment Orientation
    temp = IMUPlacementEst(2).inv * q_diag2(i) * IMUPlacementEst(1) / measurement(1) * measurement(2);
    dr_du_j1(:, i) = 2 * temp.v';
    
    %% IMU Placement2
    temp = q_diag2(i) / rotation * IMUPlacementEst(1) / measurement(1) * measurement(2);
    dr_du_s2(:, i) = 2 * temp.v';
end
