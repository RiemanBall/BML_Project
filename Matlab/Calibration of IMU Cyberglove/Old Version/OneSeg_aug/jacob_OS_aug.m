...
function J = jacob_OS_aug(r, x_est, measurement, constVarNum)
...
for i = 1:4
    %% IMU Placement1
    temp = IMUPlacementEst(2).inv * q_diag1(i) / rotation * measurement;
    dr_du_s1(:, i) = 2 * temp.v';
    
    %% IMU Placement2
    temp = q_diag2(i) * IMUPlacementEst(1) / rotation * measurement;
    dr_du_s2(:, i) = 2 * temp.v';
    
    %% Segment Orientation
    temp = IMUPlacementEst(2).inv * IMUPlacementEst(1) * q_diag2(i) * measurement;
    dr_du_j1(:, i) = 2 * temp.v';
end
...
if nargin == 4                      % Not fixed the joint axis
    alpha = norm(x_j1);
    if alpha < tol
        S = 1/2 - alpha^2/48;
        C = -1/24 + alpha/960;
